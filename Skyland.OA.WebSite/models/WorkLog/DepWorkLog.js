...
$.iwf.register(new function () {
    var me = this;
    this.options = { key: 'depWorkLog' };
    this.show = function (module, root) {
        $.Biz.depWorkLog.show(module, root);
    }
});


$.Biz.depWorkLog = new function () {
    var self = this;
    var root;
    self.data = null;
    var data;
    ...
    this.show = function (module, _root) {
        ...
        root.load("models/WorkLog/DepWorkLog.html", function () {
    ...
    function loadData(beginTime, endTime) {
        ...
        $.fxPost("B_WorkLogSvc.data?action=GetDataForDepWorkLog", { beginTime: beginTime, endTime: endTime, workLogType: "2" }, function (ret) {
            self.data = ret.data;
            if (ret.data) {
    ...
    function saveData(lawData, win) {
        ...
        var msg = "";
        $.post('B_WorkLogSvc.data?action=SaveData', { JsonData: content, workLogType: "2" }, function (res) {
            var json = eval('(' + res + ')')
            if (!json.success) {
                  $.Com.showMsg(json.msg);//未成功弹出错误信息
            }
            else {
                win.close();//成功关闭窗体
            }
