$.Biz.conferenceRoomPage = new function () {
    ...
    this.show = function (module, _root) {
        ...
        root.load("models/DataBaseModel/conferenceRoomPage.html", function () {
            ...
            root.find("[data-id='save']").bind("click", function () {
                ...
                //if (!callbackforsave) {
                //    var dataRow = data;
                //    gridModel.viewModel.addRow(dataRow);
                //}
                //else {
                //    var dataRow = data;
                //    callbackforsave(dataRow);
                //}
                showListDIV();
            });
    ...
    function loadData() {
        var content = "";
        $.fxPost("Para_ConferenceRoomSvc.data?action=GetData", content, function (ret) {
            if (ret) {
                data = ret;
                gridModel = $.Com.GridModel({
                   ...
                   , removeConfirm: function (row, callback) {
                       $.Com.confirm("确定删除吗？", function () {
                           deleteData(row.ConferenceRoomID());
                           callback();
                       });
                   }
                    , keyColumns: "ConferenceRoomID"// 改
                });
    ...
    // 保存数据
    function saveData(roomData) {
        if (roomData == null || roomData == "") return;
        var params = JSON.stringify({ "baseInfo": roomData });
        $.fxPost('Para_ConferenceRoomSvc.data?action=Save', { content: params }, function (ret) {
            if (ret.success)
            {
                $.Com.showMsg("保存成功！");
    ...
    function deleteData(id) {
        if (!id) return;
        var param = id;
        $.fxPost("Para_ConferenceRoomSvc.data?action=DeleteData", { content: param }, function (ret) {
            if (ret.success) {
                detailmodel.show(root.find("[data-id='detailDiv']"), ret.data.sourceListEdit);
                return true;
            } else {
...
$.iwf.register(new function () {
    var me = this;
    this.options = { key: 'conferenceRoom' };
    this.show = function (module, root) {
        $.Biz.conferenceRoomPage.show(module, root);
    };

    this.resize = function (s) {
        $.Biz.conferenceRoomPage.resize(s);
    }
});

//$.Biz.conferenceRoomPage.prototype.version = "1.0";
