$.Biz.lawRegulationPage = new function () {
    ...
    this.show = function (module, _root) {
        ...
        root.load("models/DataBaseModel/lawRegulationPage.html", function () {
    ...
    function loadData() {
        var content = "";
        $.fxPost("LawRegulationsOperationSvc.data?action=GetData", content, function (ret) {
            if (ret) {
                data = ret;
    ...
    function saveData(lawData) {
        ...
        var params = JSON.stringify({ "baseInfo": lawData });
        $.fxPost('LawRegulationsOperationSvc.data?action=Save', { content: params }, function (ret) {
            if (ret.success)
            {
                gridModel.show(root.find("[data-id='listDiv']"), ret.data.sourceList);
                detailmodel.show(root.find("[data-id='detailDiv']"), ret.data.sourceListEdit);
    ...
    function deleteData(id) {
        if (!id) return;
        var param = id;
        $.fxPost("LawRegulationsOperationSvc.data?action=DeleteData", { content: param }, function (ret) {
            if (ret.success) {
                detailmodel.show(root.find("[data-id='detailDiv']"), ret.data.sourceListEdit);
                return true;
            } else {
...
$.iwf.register(new function () {
    var me = this;
    this.options = { key: 'lawRegulation' };
    this.show = function (module, root) {
        $.Biz.lawRegulationPage.show(module, root);
    };

    this.resize = function (s) {
        $.Biz.lawRegulationPage.resize(s);
    }
});
