$.Biz.bizTypeDictionaryPage = new function () {
    ...
    this.show = function (module, _root) {
        ...
        root.load("models/DataBaseModel/bizTypeDictionaryPage.html", function () {
    ...
    function loadData() {
        var content = "";
        $.fxPost("Para_BizTypeDictionarySvc.data?action=GetData", content, function (ret) {
            if (ret) {
                data = ret;
    ...
    // 保存数据 
    function saveData(bizTypeDicData, callbackforsave) {
        if (bizTypeDicData == null || bizTypeDicData == "") return;
        var params = JSON.stringify({ "baseInfo": bizTypeDicData });
        $.fxPost('Para_BizTypeDictionarySvc.data?action=Save', { content: params }, function (ret) {
            
            if (ret.success) {
                $.Com.showMsg("保存成功！");
                detailmodel.show(root.find("[data-id='detailDiv']"), ret.data.sourceListEdit);
                //if (!callbackforsave) {
                //    gridModel.viewModel.addRow(bizTypeDicData);
                //}
                //else {
                //    callbackforsave(bizTypeDicData);
                //}
    ...
    function deleteData(keyId) {
        if (!keyId) return;
        var params = keyId;
        $.fxPost("Para_BizTypeDictionarySvc.data?action=DeleteData", { content: params }, function (ret) {
            if (ret.success) {
                detailmodel.show(root.find("[data-id='detailDiv']"), ret.data.sourceListEdit);
                return true;
            } else {
...
$.iwf.register(new function () {
    var me = this;
    this.options = { key: 'bizTypeDictionary' };
    this.show = function (module, root) {
        $.Biz.bizTypeDictionaryPage.show(module, root);
    };

    this.resize = function (s) {
        $.Biz.bizTypeDictionaryPage.resize(s);
    }
});
