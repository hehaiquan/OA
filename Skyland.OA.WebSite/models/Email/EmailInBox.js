...
$.iwf.register(new function () {
    var me = this;
    this.options = { key: 'emailinbox' };
    this.show = function (module, root) {
        $.Biz.emailinbox.show(module, root);
    };
});
$.Biz.emailinbox = new function () {
    var root;
    var data;
    var self = this;
    var models = {};
    ...
    //门户选中查看
    this.fromGateView = function (param, type) {
        this.fromGateViewInit(param, type);
    }

    this.fromGateViewInit = function (param, type) {
        choiceData = param;
        if (param.Mail_IsSee == "0") {
            //设置为已查看
            UpdateB_EmailByIdSetMail_IsSee(param.ID, '1');
        }

        //可编辑弹窗
        param.emailRecieveDate = $.Com.formatDate(param.emailRecieveDate);
        var callback = "";
        showEmailDetailPage(param, callback);
    }
    ...
    models.gridModel = $.Com.GridModel({
        ...
        beforeBind: function (vm, _root) {//表格加载前
            ...
            vm.details = function (id) {
                  $.Com.showMsg(id());
                ko.observable("Report including final year-end statistics")
            }
            ...
            vm.getIsCollection = function (isCollection) {
                if (isCollection() == false) {
                    ...
                    return "<img src='../resources/images/colums/email/unblind_start.png' data-toggle='tooltip' title='未标星'/>";
                } else {
                    //return "<span style=\"color:#339900\">已读</span>";
                    return "<img src='../resources/images/colums/email/start.png' data-toggle='tooltip' title='标星'/>";
                }
            }
            ...
            vm.setIsCollection = function (id, isCollection) {
                ...
                for (var i = 0; i < data.dataList.length; i++) {
                    if (data.dataList[i].ID == emailId) {
                        ...
                        models.gridModel.show(root.find('[data-role="inBoxGrid_Part"]'), data.dataList);
                        break;
                    }
                }
            }
    ...
    this.show = function (module, _root) {
        $.Biz.emailinbox.isInitial = true;
        root = _root;
        if (root.children().length != 0) return;
        root.load("models/Email/EmailInBox.html", function () {
            ...
            if ($.Biz.emailinbox.type == 'view') {
                $.Biz.emailinbox.fromGateViewInit($.Biz.emailinbox.choiceData, 'view');
            }
            ...
            root.find("[data-id='deleteBtn']").bind("click", function () {
                if (!confirm("确定要删除邮件吗？")) return false;
                var deleteList = [];
                var cacheData = models.gridModel.getCacheData().data;//取出表中改变的字段.
                for (var i = 0; i < cacheData.length; i++) {
                    if (cacheData[i].isSelected == true) {
            ...
            root.find("[data-id='thoroughDeleteBtn']").bind("click", function () {
                if (!confirm("确定要删除邮件吗,若删除后将无法恢复。")) return false;
                var deleteList = [];
                var cacheData = models.gridModel.getCacheData().data;//取出表中改变的字段.
                for (var i = 0; i < cacheData.length; i++) {
                    if (cacheData[i].isSelected == true) {
    ...
    //载入数据
    function loadData() {
        $.fxPost("B_Email_One_Svc.data?action=GetEmailInboxInit", "", function (ret) {
            if (ret.data) {
                data = ret.data;
                models.gridModel.show(root.find("[ data-id='inBoxGrid_Part']"), data.dataList);
                models.countModel.show(root.find("[data-id='tittleInform']"), data.dataCount);
                emailMarkModel = data.emailMarkModel;
                emailDocumentModel = data.emailDocumentModel;
            }
        });
    }
    ...
    function showEmailDetailPage(item, callback) {
        root.find("[data-id='detailView_Part']").attr("style", "display:display;");
