...
$.iwf.register(new function () {
    var me = this;
    this.options = { key: 'emailgabbagebox' };
    this.show = function (module, root) {
        $.Biz.emailgabbagebox.show(module, root);
    };
});

$.Biz.emailgabbagebox = new function () {
    var root;
    var data;
    var self = this;
    var models = {};
    ...
    models.gridModel = $.Com.GridModel({
        ...
        beforeBind: function (vm, _root) {//表格加载前
            ...
            //                    models.gridModel.show(root.find('[data-role="emailGabbageTableGrid"]'), data.dataList);
            //                    break;
            //                }
            //            }a
            //            //刷新未读取数量
            //            models.countModel.show(root.find("[data-id='tittleInform_EmailGabbage']"), ret.data);
            //        }
            //    })
            //}
    ...
    this.show = function (module, _root) {
        ...
        root.load("models/Email/EmailGabbageBox.html", function () {
            ...
            root.find("[data-id='changeSend_Two']").bind("click", function () {
                ...
                $.Biz.emailwritebox.url = 'm2.emailgabbagebox:{type:"gabbagebox"}';
    ...
    //载入数据
    function loadData() {
        $.fxPost("B_Email_One_Svc.data?action=GetDeleteEmail", "", function (ret) {
            if (ret.data) {
                data = ret.data;
                models.gridModel.show(root.find("[ data-id='inBoxGrid_Part']"), data.dataList);
                models.countModel.show(root.find("[data-id='tittleInform_EmailGabbage']"), data.dataCount);
            }
        });
    }
        ...
        var dlgOpts = {
            ...
            button: [
           {
               text: '确定', handler: function (data) {
                   ...
                   //添加标记
                   $.fxPost("B_Email_One_Svc.data?action=SaveEmailMark", { JsonData: da.markName }, function (ret) {
                       if (!ret.success) {
                             $.Com.showMsg(ret.msg);
                       }
                       //重新读取
    ...
    function UpdateB_EmailByIdSetMail_IsSee(ID, mailIsSee) {
        //将邮件变为已读
        $.fxPost("B_Email_One_Svc.data?action=UpdateB_EmailByIdSetMail_IsSee", { id: ID, mailIsSee: mailIsSee, emailType: "chaogao" }, function (ret) {
            if (ret.success) {
                for (var i = 0; i < data.dataList.length; i++) {
