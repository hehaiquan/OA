...
$.Biz.UserSingleSelectControlInfor = function (el, valueAccessor) {
    var self = this;
    var root;
    var models = {};
    ...
    models.gridModel = $.Com.GridModel({
        ...
        beforeBind: function (vm, _root) {//表格加载前
            vm._deleteData = function (rowData) {
                var check;
                if (!rowData.id) {
                    check = self.ztr.getNodeByParam("name", rowData.name());
                    selectData.username = "";

                } else {
                    check = self.ztr.getNodeByParam("id", rowData.id());
                    selectData.username = "";
                    selectData.id = "";
                    selectData.dpname = "";
                }
    ...
    this.show = function (module, _root) {
        ...
        root.load("models/CommonControl/UserSingleSelectControl.html", function () {
            //初始化选择表
    ...
    //将树状图已选中的check设置为非check状态
    function zTreeBeforeCheck() {
        var nodes = self.ztr.getNodesByParam("checked", true);
        for (var i = 0 ; i < nodes.length; i++) {
            setTreeIsUnCheck(nodes[i]);
        }
    }
    ...
    // 将所有树节点设置为非选中状态
    function setTreeIsUnCheck(node) {
        node.checked = false;
        self.ztr.updateNode(node);
        self.ztr.checkNode(node, false, false);
        if (node.children && node.children.length != 0) {
            for (var i = 0; i < node.children.length; i++) {
                setTreeIsUnCheck((node.children)[i]);
            }
        }
    }
    ...
    function loadData(searchType) {
        $.fxPost("UserSelectControlSvc.data?action=GetData", {}, function(res) {
            if (!res.success) {
                $.Com.showMsg(res.msg);
                return;
            }
            ...
            require(["script/ztree/jquery.ztree.exhide-3.5.js"], function() {
                ...
                //将所有父节点设置为不可选
                var parentNode = self.ztr.getNodesByParam("isParent", true);
                for (var p = 0; p < parentNode.length; p++) {
                    parentNode[p].nocheck = true;
                    self.ztr.updateNode(parentNode[p]);
                }
                ...
                if (valueAccessor.userid() != "" && valueAccessor.userid() != undefined) {
                    var useridArray = valueAccessor.userid();
                    var node = self.ztr.getNodesByParam("id", useridArray);
                    node[0].checked = true;
...
//加入用户列表的弹窗
$.Biz.UserSingleSelectControl = function (element, valueAccessor, callback) {
    var model = new $.Biz.UserSingleSelectControlInfor(element, valueAccessor);
    var root = null;
    var opts = {
        title: '选择用户',
        height: 730,
        ...
        button: [
           {
               text: '确定', handler: function (data) {
                   var d = model.getData();
                   if (valueAccessor.userid != "") valueAccessor.userid(d.userid);
