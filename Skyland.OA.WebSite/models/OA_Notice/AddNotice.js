$.iwf.register(new function () {
    var me = this;
    this.options = { key: 'addnotice' };
    this.show = function (module, root) {
        $.Biz.addnotice.show(module, root);
    };
});

$.Biz.addnotice = new function () {
    var root;
    var self = this;
    var models = {};
    var editor;
    ...
    var noticeModel;
    var textarea = "<textarea class='form-control' name='aticleText_AddNotice' data-bind='value:NewsText' rows='16'></textarea>";
    var editType;
    var editData;
    //返回按钮的连接地址
    var url = ""; 
    ...
    this.show = function (module, _root) {
        $.Biz.addnotice.isInitial = true;
        root = _root;
        if (root.children().length != 0) return
        root.load("models/OA_Notice/AddNotice.html", function () {
            $.fxPost("DocumentCenterSvc.data?action=GetPubliceModel", { getTypeName: "publishNotice" }, function (ret) {

                if (!editor) {
                    var aticleText_AddNotice = root.find("[data-id='aticleText_Update']");
                    aticleText_AddNotice.empty(); //删除被选元素的子元素。
                    aticleText_AddNotice.append(textarea);
                    editor = CKEDITOR.replace('aticleText_AddNotice');
                }
            ...
            root.find("[data-id='publicRange']").bind("change", function () {
                ...
                var da = models.publishAticleModel.getData();
                if (!da && da == null) {
                    return false;
                }
