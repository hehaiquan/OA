...
$.iwf.register(new function () {
    var me = this;
    this.options = { key: 'addpublish' };
    this.show = function (module, root) {
        $.Biz.addpublish.show(module, root);
    }
});
...
$.Biz.addpublish = new function () {
    ...
    var noticeModel;
    var textarea = "<textarea class='form-control' name='aticleText_Addpublish' data-bind='value:NewsText' rows='16'></textarea>";
    var editType;
    var editData;
    //返回按钮的连接地址
    var url = "";
    ...
    function loadSelect(data) {
        ...
        //循环创建菜单
        for (var i = 0; i < data.length; i++) {
            var divItem = $("<div title='' style='float: left' data-id='" + data[i].id + "'> <b>" + data[i].name + "</b><img data-id='im" + i + "' src='../../resources/images/docCenter/delete.png' /></div>");
    ...
    this.show = function (module, _root) {
        $.Biz.addpublish.isInitial = true;
        root = _root;
        if (root.children().length != 0) return;
        root.load("models/OA_Notice/Addpublish.html", function () {
            $.fxPost("DocumentCenterSvc.data?action=GetPubliceModel", { getTypeName: "addpublish" }, function (ret) {

                if (!editor) {
                    var aticleText_Addpublish = root.find("[data-id='aticleText_Update']");
                    aticleText_Addpublish.empty(); //删除被选元素的子元素。
                    aticleText_Addpublish.append(textarea);
                    editor = CKEDITOR.replace('aticleText_Addpublish');
                }
                ...
                if ($.Biz.addpublish.type == 'edit') {
                    url = $.Biz.addpublish.url;
                    $.Biz.addpublish.editAticleInit($.Biz.addpublish.editData, 'edit');
                } else if ($.Biz.addpublish.type == 'new') {
                    $.Biz.addpublish.editAticleInit($.Biz.addpublish.editData, 'new');
                } else {
    ...
    //读取数据
    function loadData() {
        $.fxPost("DocumentCenterSvc.data?action=GetPubliceModel", { getTypeName: "publishNotice" }, function (ret) {
            noticeModel = ret.data;
            models.publishAticleModel.show(root.find("[data-id='baseInfo']"), noticeModel);
