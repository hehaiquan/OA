define(new function() {
    ...
    //门户跳转
    this.fromGateway = function sendGrid(param, type) {
    ...
    this.show = function (module, _root) {
        ...
        root.load("nnepb/OA/SearchAndStatic/OASendDocSearchPage.html", function () {
            //读取发文所有按钮
            loadSendBtn();
            loadData("");
        });
    ...
    function loadData(condition) {
        var modelDIV = root.find("[data-id='sendGrid']");
        var url = "";
        url = "nnepb/OA/SearchAndStatic/OASendDocSearchGrid";
        ...
        $.fxPost("OASendDocSearchSvc.data?action=getBtnArray", "", function (ret) {
            ...
            for (var i = 0 ; i < datalist.length; i++) {
                ...
                btnDiv.find("[data-id='" + datalist[i].FileTypeId + "']").bind("click", function () {
                    ...
                    var buttonArray = root.find("[data-id='btnSendArray']")[0].getElementsByTagName("button");
                    root.find("[data-id='btnSendArray']").find("button").css("background-color", "#FFF");
                    root.find("[data-id='btnSendArray']").find("button").css("color", "#000000");
                    //将所选字段设置为有背景色并且是白色字体
    ...
    function setBackgoundBlue(typeName) {
        root.find("[data-id='btnSendArray']").find("button").each(function (index, item) {
