    ...
    models.gridModel = $.Com.GridModel({
        beforeBind: function(vm, _root) {
            ...
            vm.editInfor = function(caseid) {
                $.fxPost("B_OA_MeetingSvc.data?action=GetMeetingByCaseId", { caseid: caseid }, function (ret) {
                    showEditDocumentWind(ret.baseInfor);
                });
            }
            vm._deleteRow = function(caseId) {
                if (confirm("您确定要删除此项信息吗？")) {
                    $.fxPost("B_OA_MeetingSvc.data?action=DeleteData", { caseId: caseId }, function (ret) {
                        loadData();
                    });
                }
            }
...
//编辑文件夹弹窗
            ...
            button: [
           {
               text: '确定', handler: function () {
                   ...
                   if (da) {
                       ...
                       $.fxPost("B_OA_MeetingSvc.data?action=SaveData", { jsonData: json }, function (ret) {
                           loadData();
                           win.close();
    ...
    this.show = function(module, _root) {
        ...
        root.load("nnepb/OA/MeetingManage/MeetingApplyGrid.html", function () {
            loadData();
        });
    }

    function loadData() {
        $.fxPost("B_OA_MeetingSvc.data?action=GetMeetingGrid", { }, function (ret) {
