define(new function () {
    var me = this;
    this.options = { key: 'dailyrecievedocument' };
    var models = {};
    ...
    this.show = function (module, root) {
        if (root.children().length == 0) {
            root.load("nnepb/portlet/OA/DailyReceiveDocumentPage.html", function () {
                root.css({ 'paddingTop': '5px' });
                //$.post("DailyReceiveDocumentSvc.data?action=QueryMyCase", {}, function (ret) {
                //    if (ret) {
                ...
                jQuery.PackResult("engine.data?action=Querymycase&ttt=" + Math.random(), {}, function (data) {
                    ...
                    for (var i = 0; i < 3; i++) {
                        ...
                        for (var j = 0; j < data[i].children.length; j++) {
                            ...
                            if (data[i].children[j].FlowID == "W000066"
                    ...
                    var DataListDiv = root.find("[data-id='myDailyReceiveDocumentDataList']");
                    var divHtml ="" ;
