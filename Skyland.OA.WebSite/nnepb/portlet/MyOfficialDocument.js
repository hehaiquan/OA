define(new function () {
    var me = this;
    this.options = { key: 'MyOfficialDocument' };
    var models = {}
    ...
    this.show = function (module, root) {
        if (root.children().length == 0) {
            //行政办公
            root.load("nnepb/portlet/MyOfficialDocument.html", function () {
                root.css({ 'paddingTop': '5px' });
                //$.post("engine.data?action=Querymycase&ttt=" + Math.random(), {}, function (data) {
                jQuery.PackResult("engine.data?action=Querymycase&ttt=" + Math.random(), {}, function (data) {
                    ...
                    var DataListDiv = root.find("[data-id='myOfficialDocumentDataList']");
                    var divHtml ="" ;
