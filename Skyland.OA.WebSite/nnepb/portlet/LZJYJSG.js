define(new function () {
    ...
    this.show = function (module, root) {
        if (root.children().length == 0) {
            var typeId = "LZJYJSG";
            root.load("nnepb/portlet/LZJYJSG.html", function () {
                root.css({ 'paddingTop': '5px' });
                $.fxPost('DocumentCenterSvc.data?action=GetUserGateForType', { top: 5, documentType: "廉政警示教育馆" }, function (res) {
                    if (!res.success) {
                          $.Com.showMsg(res.msg);
                        return;
                    }
                    ...
                    ViewMorelOnclick_LZJYJSG = function () {
                    ...
                    var url_dbx = 'index.html#m2.noticeSee:{title:"廉政教育警示馆查看"}'; //公告查询
                    var DataListDiv = root.find("[data-id='myLZJYSGList']");
                    var divHtml = "";
                    //divHtml += "<tr >";
                    //divHtml += "<td><span style='line-height: 30px; color: #000000;'>&nbsp;&nbsp;</span><a onclick=ViewMorelOnclick_LZJYJSG('')><span class='more'>更多>></span></a></td>";
                    //divHtml += "</tr>";
