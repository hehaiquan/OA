define(new function () {
    ...
    this.show = function (module, root) {
        if (root.children().length == 0) {
            var typeId = "tzgg";
            root.load("nnepb/portlet/Learning.html", function () {
                root.css({ 'paddingTop': '5px' });
                $.fxPost('DocumentCenterSvc.data?action=GetUserGateForType', { top: 5, documentType: "学习园地" }, function (res) {
                    if (!res.success) {
                          $.Com.showMsg(res.msg);
                        return;
                    }
                    ...
                    ViewMorelOnclick_Learning = function () {
                    ...
                    var url_dbx = 'index.html#m2.noticeSee:{title:"学习园地查看"}'; //公告查询
                    var DataListDiv = root.find("[data-id='myNoticeDataList']");
                    var divHtml = "";
                    //divHtml += "<tr >";
                    //divHtml += "<td><span style='line-height: 30px; color: #000000;'>&nbsp;&nbsp;</span><a onclick=ViewMorelOnclick_Learning('')><span class='more'>更多>></span></a></td>";
                    //divHtml += "</tr>";
