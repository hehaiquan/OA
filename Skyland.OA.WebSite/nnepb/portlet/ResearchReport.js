define(new function () {
    var me = this;
    this.options = { key: 'ResearchReport' };
    var models = {};
    var fileType;
    ...
    this.show = function (module, root) {
        if (root.children().length == 0) {
            var typeId = "tzgg";
            root.load("nnepb/portlet/ResearchReport.html", function () {
                root.css({ 'paddingTop': '5px' });
                $.fxPost('DocumentCenterSvc.data?action=GetUserGateForType', { top: 5, documentType: "调研报告" }, function (res) {
                    if (!res.success) {
                          $.Com.showMsg(res.msg);
                        return;
                    }
                    ...
                    ViewMorelOnclick_ResearchReport = function () {
                    ...
                    var DataListDiv = root.find("[data-id='researchReportList']");
                    var divHtml = "";
                    //divHtml += "<tr >";
                    //divHtml += "<td><span style='line-height: 30px; color: #000000;'>&nbsp;&nbsp;</span><a onclick=ViewMorelOnclick_ResearchReport('')><span class='more'>更多>></span></a></td>";
                    //divHtml += "</tr>";
