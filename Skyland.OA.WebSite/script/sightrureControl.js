(function ($) {
    $.fn.SightureContorl = function (options) {
        ...
        qf_textarea += "<button class='btn btn-default' data-id='saveSighture_" + opts.RecordID + "' type='button' style='margin-bottom: 30px; margin-left: 5px;' >保存</button>";
        qf_textarea += " </td>";
        qf_textarea += "</tr>";
        qf_textarea += " </table>";

        root.append(qf_textarea);
        ...
        var FieldName = encodeURI(opts.FieldName);
        SendOut.FieldName = FieldName//表中的字段名
        ...
        root.find("[data-id='saveSighture_" + opts.RecordID  + "']").click(function () {
            ...
            var name = opts.documentName + '/' + opts.RecordID + ".jpg";
            opts.RemarkBox_DOM.val(name);
            //setCheckSuggetion(imageDiv, opts.RecordID, columnsName);
            //图片显示
            //opts.imageDiv.empty();
            //var image = "<img src='" + opts.showPath + '/' + opts.RecordID + ".jpg' />";
            //opts.imageDiv.append(image);
        });

    };

    function setCheckSuggetion(imageDiv, caseId, columnName) {
        $.fxPost("B_OA_SinghtureSvc.data?action=GetSighture", { caseId: caseId, columnName: columnName }, function (ret) {
            var sightureList = ret.dt;
            imageDiv.empty();
