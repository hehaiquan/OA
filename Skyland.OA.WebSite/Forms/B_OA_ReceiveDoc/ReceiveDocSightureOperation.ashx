<%@ WebHandler Language="C#" Class="ReceiveDocSightureOperation" %>
...
public class ReceiveDocSightureOperation : Page
{
    ...
    public bool LoadSignature()
    {
        ...
        try
        {
            ...
            strSql.AppendFormat(@"select * from B_OA_Sighture where caseid='{0}' and tableName='{1}' and columnName = '{2}' and type='{3}'", mRecordID, "ReceiveDoc", "ldps", "0");
    ...
    /// <returns></returns>
    public bool DeleteReceiveDoc(string caseid, string tableName, string columnName, string userid, IDbTransaction tran)
    {
        StringBuilder strSql = new StringBuilder();
    ...
    public string SaveSighture(string columnName)
    {
        ...
        try
        {
            ...
            //删除原来的文件和相关数据
            DeleteReceiveDoc(mRecordID, "ReceiveDoc", mFieldName, mUserName, tran);

            //保存新的签名和文件入文件夹中
            string fileName = mRecordID + "_" + mFieldName + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".jpg";//服务器的文件保存名
            ...
            //修改图片尺寸
            //string fileName_edit = mRecordID + "_" + mFieldName + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_edit" + ".jpg";//服务器的文件保存名
            ...
            //sighture_edit.tableName = "ReceiveDoc";
