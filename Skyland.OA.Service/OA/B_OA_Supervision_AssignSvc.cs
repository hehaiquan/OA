...
{
    class B_OA_Supervision_AssignSvc : BaseDataHandler
    {
        ...
        public string GetSupervisionBill(string type)
        {
            ...
            {
                billDiv.Append(@"
                         <p style='text-align: left;'><span style='font-size:16pt'><u>&nbsp; &nbsp;&nbsp; &nbsp;:&nbsp;</u></span></p>
                           <p style='text-align: left;'><span style='font-size:16pt'>根据<u>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</u>（&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;）
        ...
        public string SaveSupervsionMsg(string content, string userid)
        {
            ...
            try
            {
                ...
                dataModel.supervisionMsg.userName = ComClass.GetUserInfo(userid).CnName;
                dataModel.supervisionMsg.createDate = DateTime.Now.ToString();
                dataModel.supervisionMsg.caseId = supervisionMsg.caseId;
                Utility.Database.Commit(tran);
                return Utility.JsonResult(true, "保存成功", dataModel);
            }
            catch (Exception e)
        ...
        [DataAction("GetMySupervision", "userid")]
        public object GetMySupervision(string userid)
        {
            IDbTransaction tran = Utility.Database.BeginDbTransaction();
            StringBuilder strSql = new StringBuilder();
            try
...
select ROW_NUMBER() over (PARTITION BY a.caseid order by a.createDate desc) as ROWINDEX,
	a.caseId,
	a.undertake_Department,
	a.manageDate,
	a.limiteDate,
	a.assistDepartment,
	a.supervisionManName,
	a.supervisionManId,
	a.assistMan,
	a.assitManId,
	a.createDate,
	...
	a.title,
    a.Name,
    case when a.mc IS NULL then '' else a.mc end  as enmergencyLevel
	into #Temp
	from(
	select a1.*,b1.ActID,c1.FlowName,c1.Name,em.mc from FX_WorkFlowBusAct as b1 
			left join B_OA_Supervision  as a1 on a1.caseId = b1.CaseID
			left join FX_WorkFlowCase as c1 on a1.caseId = c1.ID
			left join (select a.csz as id, a.mc as mc from Para_BizTypeItem a , 
						Para_BizTypeDictionary b where a.flid = b.id and b.lx = 'emergencyLevelDic') as em
		    on em.id = a1.enmergencyLevel
			) as a
	where a.ActID = 'A003' and
	(a.assitManId like '%{0}%' or a.supervisionManId like '%{0}%')
    and
    a.caseid is not null
	order by a.manageDate desc
SELECT  *
FROM    #Temp
WHERE   ROWINDEX = 1
DROP TABLE #Temp;
...
DROP TABLE #Temp2;
        ...
        [DataAction("GetAllSupervision", "userid")]
        public object GetAllSupervision(string userid) { 
         IDbTransaction tran = Utility.Database.BeginDbTransaction();
            StringBuilder strSql = new StringBuilder();
            try
            {
...
select ROW_NUMBER() over (PARTITION BY a.caseid order by a.createDate desc) as ROWINDEX,
	...
	into #Temp
	from(
	select a1.*,b1.ActID,c1.FlowName,c1.Name,b1.ActName,em.mc from FX_WorkFlowBusAct as b1 
			left join B_OA_Supervision  as a1 on a1.caseId = b1.CaseID
			left join FX_WorkFlowCase as c1 on a1.caseId = c1.ID
			left join (select a.csz as id, a.mc as mc from Para_BizTypeItem a , 
						Para_BizTypeDictionary b where a.flid = b.id and b.lx = 'emergencyLevelDic') as em
...
");
        ...
        public override string Key
        {
            get
            {
                return "B_OA_Supervision_AssignSvc";
            }
        }
    }
}
